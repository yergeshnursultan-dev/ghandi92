...
module.exports = async function handler(req, res) {
  if (req.method !== 'POST') {
    return send(res, 405, { ok: false, message: 'POST әдісі қажет' });
  }
  ...
  try {
    ...
    const imagePrompt = `Create a highly aesthetic dreamy fantasy illustration, soft pink clouds, luminous pastel atmosphere, elegant school-safe surreal concept art.
No text, no letters, no logo, no watermark. Main imagination: ${imagination}. Mood: ${mood}. Color palette: ${palette}. World: ${world}. Character preference: ${character}. Extra details: ${details}. Style: cinematic soft light, glassy glow, magical clouds, gentle composition, beautiful emotional visual, high quality.`;

    const textPrompt = `Пайдаланушы қиялындағы бейнені сипаттады. Сипаттаманы қазақ тілінде бер. Стиль: жылы, жұмсақ, шабыттандыратын, мектепке қауіпсіз.
Ешқандай диагноз, медициналық қорытынды, ауыр психологиялық болжам жасама. Тек шығармашылық рефлексия ретінде түсіндір.
Міндетті бөлімдер: 1) Қиялдағы негізгі образ, 2) Түстер мен көңіл күй, 3) Шығармашылық мінездеме, 4) Қысқа шабытты қорытынды.
...
Деталь: ${details}`;
    ...
    const description =
      ...
      'Қиял бейнесі жұмсақ, арманшыл және шығармашылық бағытта сипатталады.';

    if (!b64) {
      throw new Error('Сурет base64 форматында келмеді');
    }
